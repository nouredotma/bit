...
<div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mb-10">
    <div class="bg-gray-50/50 p-6 rounded-2xl border border-gray-100">
        <p class="text-[10px] uppercase tracking-widest text-gray-400 font-bold mb-2">Total Revenue</p>
        <p class="text-3xl font-bold tracking-tighter">${{ number_format($stats['totalRevenue'], 2) }}</p>
    </div>
    <div class="bg-gray-50/50 p-6 rounded-2xl border border-gray-100">
        <p class="text-[10px] uppercase tracking-widest text-gray-400 font-bold mb-2">Total Orders</p>
    ...
    <div class="bg-gray-50/50 p-6 rounded-2xl border border-gray-100">
        <p class="text-[10px] uppercase tracking-widest text-gray-400 font-bold mb-2">Products</p>
...
<div class="flex justify-between items-center mb-6">
    ...
    <a href="{{ route('admin.orders.index') }}" class="text-xs uppercase tracking-widest font-bold text-gray-400 hover:text-black transition-colors">View All →</a>
</div>
    ...
    <table class="w-full text-left">
        ...
        <tbody class="divide-y divide-gray-50">
            ...
            <tr class="hover:bg-gray-50/50 transition-colors">
                <td class="py-4 font-bold text-sm">#{{ str_pad($order->id, 6, '0', STR_PAD_LEFT) }}</td>
                <td class="py-4 text-sm">{{ $order->customer_name }}</td>
                <td class="py-4 font-bold text-sm">${{ number_format($order->total, 2) }}</td>
                <td class="py-4">
                    @php
